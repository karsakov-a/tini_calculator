...
## Описание проекта
CITI (Composite Inflammatory–Thrombotic Index) Calculator — это автономное настольное приложение для врачей и клиницистов, предназначенное для быстрого расчёта тромбо-воспалительного индекса (CITI) — прогностического показателя, оценивающего риск неблагоприятного исхода у пациентов с тяжёлыми инфекционными и воспалительными состояниями.
...
Программа автоматически интерпретирует результат:
  ...
  - При наличии данных КТ (>70% поражения лёгких) и CITI > 500 000 система выводит дополнительное предупреждение о чрезвычайно высоком риске.
...
6. Работа с журналом расчётов:
  - Все выполненные расчёты автоматически сохраняются в локальный журнал.
  - Для просмотра журнала нажмите кнопку «Журнал» в нижней части окна.
  - В журнале отображаются:
  - Дата и время расчёта
  - ФИО пациента
  - Даты рождения и исследования
  - Значение CITI и уровень риска
  - Вы можете:
  - Открыть запись — загрузить её обратно в основную форму для просмотра или повторного экспорта
  - Удалить запись — из журнала и истории
  - Экспортировать весь журнал в CSV-файл (кодировка UTF-8 с BOM для корректного отображения в Excel)

## Установка
1. Клонирование репозитория:
```
git clone https://github.com/your-repo/CITI_calculator.git 
cd CITI_calculator
```
...
```
Будет запущена демо версия программы с текущим состоянием кода.
